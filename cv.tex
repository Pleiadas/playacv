...
% created by Martin Knöfel (https://github.com/Pleiadas/), v1.3, Nov 15, 2023
...
% you might set up this file on first use with all your default information and settings
%%% custom settings in your.tex (see template.tex example) overwrite the settings first initialised here
...
% default document layout settings, 1 = include, 9 = no

% default header settings
% if you use optional pages, you can overwrite headers settings before they are input in your.tex with \renewcommand{HeaderSetting}{New Value}

\newcommand{\WithHeader}{1}
\newcommand{\ConfidentialLetter}{9} % <- ifnum = 1
\newcommand{\ConfidentialHeader}{-- CONFIDENTIAL --} % line appears over recipient name in letter header

% you can add one or multiple photos on the left or right with ',' e.g. qr,pic
\newcommand{\aHeaderPhoto}{appendix/snow} % photo(s) file name/s without extensions, e.g. pic or qr,pic
...
\newcommand{\eHeaderWatermark}{-0.5} % up -/+ down position in cm, useful to adjust address fields vertically
...
% cv columns default settings
% set the left/right column width ratio to 37:63.
\newcommand{\MyColumnRatio}{0.4}

% vertical alignment of the start of the left and right columns, sometimes the Title are misaligned
\newcommand{\vLeftColumn}{0.00cm}
\newcommand{\vRightColumn}{0.00cm}
...
% appendix default settings e.g. for adding additional documents to your application
\newcommand{\WithAppendix}{9} % appendixes are setup without header and footer by default
...
% form default settings, renders blank business letter form
\newcommand{\WithForm}{9} % form letter is setup with forced header and footer design, change in _chap if needed
%\newcommand{\OnlyForm}{9} % to do
\newcommand{\FormSubject}{Reply to ``\Subject`` (\myDate)}
\newcommand{\FormBody}{
	\linefield{formgreeting}{0.42\textwidth} \hfill ~\par
	\bigskip
	\boxfield[11.37cm]{formpar}{\textwidth}\par
	\vspace{.5cm}
	~ \hfill \rightfield{~~}{0.42\textwidth}\par
}

%%%%%%% LONG LIST OF DEFAULT SETTINGS IN (MOSTLY) ALPHABETICAL ODER %%%%%%%
% Set default visibility and values for your CV content below. Uses 
...
%%% you can reuse any setting with \renewcommand{\WithCvContent}{9}in your.tex to overwrite as needed

% abstract default settings
...
\newcommand{\fExperienceOne}{9} % set to 1..5 to highlight experience or 9
...
\newcommand{\fExperienceTwo}{9} % set to 1..5 to highlight experience
...
\newcommand{\HobbyFour}{1}
\newcommand{\aHobbyFour}{Cooking}

\newcommand{\HobbyFive}{9}
\newcommand{\aHobbyFive}{\LaTeX}

\newcommand{\HobbySix}{1}
\newcommand{\aHobbySix}{Symbols \faHeart}
